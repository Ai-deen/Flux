...
function Decode-Value {
    ...
    # Handle split-encoded values (pipe-separated)
    if ($encoded.Contains("|")) {
        $parts = $encoded.Split("|")
        $decoded = ""
        foreach ($part in $parts) {
            $decoded += [System.Text.Encoding]::UTF8.GetString([Convert]::FromBase64String($part))
        }
        return $decoded
    }
...
$DEMO_CONFIG = @{
    ...
    AZURE_STORAGE_CONNECTION_STRING = ""
    ...
    GROQ_API_KEY        = "Z3NrXzBQTXdEN3F3|dlhsQ2lqdlhjMVN1|V0dkeWIzRlkwSTgw|WllKQ29rN21KZnpwM25TczJEMks="
    ENGMEMORY_REPO_URL  = "aHR0cHM6Ly9naXRodWIuY29tL0FpLWRlZW4vRmx1eC5naXQ="
    FLUX_DASHBOARD_URL  = "aHR0cHM6Ly9mbHV4LTF5ZjYub25yZW5kZXIuY29t"
}
...
function Write-EnvFile {
    ...
    if ($mode -eq 1) {
        ...
        $d_groqKey = Decode-Value $DEMO_CONFIG.GROQ_API_KEY
        $d_repoUrl = Decode-Value $DEMO_CONFIG.ENGMEMORY_REPO_URL
        $d_dashUrl = Decode-Value $DEMO_CONFIG.FLUX_DASHBOARD_URL

        $lines = @(
            "# Flux - Configuration (Demo Mode)"
...
function Start-Services {
    Write-Step "6/6" "Starting Flux services..."
    ...
    Write-Host "  |                                                           |" -ForegroundColor Green
    Write-Host "  |  Live Dashboard: https://flux-1yf6.onrender.com           |" -ForegroundColor Green
    Write-Host "  |  Local API:      http://localhost:5051/docs                |" -ForegroundColor Green
    Write-Host "  ============================================================" -ForegroundColor Green
    Write-Host ""

    Start-Process "https://flux-1yf6.onrender.com"
